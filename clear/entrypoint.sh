...
if [ ! -f "/root/.ssh/id_rsa_22" ];then
    ssh-keygen -t rsa -N '' -q -f /root/.ssh/id_rsa_22
    cat /root/.ssh/id_rsa_22.pub >> /root/.ssh/authorized_keys
    _f1='{"username": "root", "pkey": "'
    _f2=$(sed s/$/'\\n'/ /root/.ssh/id_rsa_22 | tr -d '\r\n')
    _f3='", "is_save": "1", "c_type": "True", "host": "127.0.0.1", "password": "", "port": 22}'
